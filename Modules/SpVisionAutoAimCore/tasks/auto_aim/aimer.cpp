  ...
  bool to_now)
{
  if (targets.empty()) {
    ...
    debug_target_name = ArmorName::not_armor;
    debug_target_armor_type = ArmorType::small;
    return {false, false, 0, 0};
  }
  auto target = targets.front();
  debug_target_name = target.name;
  debug_target_armor_type = target.armor_type;
  ...
  for (int iter = 0; iter < 10; ++iter) {
    ...
    debug_target_name = iteration_target[iter].name;
    debug_target_armor_type = iteration_target[iter].armor_type;
    if (!aim_point.valid) {
      return {false, false, 0, 0};
    }
